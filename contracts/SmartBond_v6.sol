// TODO:Ottimizzare un pò il codice mettendo dei metodi in librerie di utililty
// TODO:Creare della documentazione
// TEST: se faccio partire in un mese con 31 gg e la cedola successiva capita in un mese con 30g, come si comporta?
...
library FixidityLib {
    ...
    function transformFixedToRealEtherValue(uint _inputFixed) public pure returns(uint) {
        return _inputFixed/fixedToRealNum;
    }
    
    // Trasformo un valore reale in Ether in un numero Fixed
    function transformRealEtherValueToFixed(uint _inputReal) public pure returns(uint) {
...
library Utility {
    ...
    /**
     * Converto un valore uint in stringa
     */
    ...
    /*
     * Funzione di verifica legittimità richiesta di pagamento in base alla data di richiesta
     */
    function checkDate(uint _timestampValidDate, uint _realDaysDeltaPaymentDate) internal view returns (bool result){
...
contract SmartBond is Ownable {
    ...
    uint fixedFrequency; // numero mesi fra una cedola a l'altra. FIXED
    uint fixedNumCedole; // numero di cedole da pagare. FIXED
    uint fixedEuroInterest; // totale interessi da pagare in Euro. FIXED
    uint fixedYears; // numero totale di anni di validità dello SmartBond. FIXED
    uint fixedNumOne; // numero 1. FIXED
	...
	uint fixedEuroEtherValue; // valore di un Ether in Euro. FIXED
    
    uint fixedEuroCedolaValue; // valore della cedola in Euro. FIXED
    uint fixedEtherCedolaValue; // valore della cedola in Ether. Valore calcolato al momento della richiesta Pay FIXED
    uint realEtherCedolaValue; // valore della cedola in Ether. Valore calcolato al momento della richiesta Pay
	
	uint fixedEuroCapitalValue; // capitale nominale versato dal contraente in Euro. FIXED
    uint fixedEtherCapitalValue; // capitale nominale versato dal contraente in Ether. FIXED
    ...
    */
    constructor (address payable _addrBeneficiary, uint _capitalEuro, uint _interestEuro, uint _years, uint _frequency, uint _startDate) public {
        ...
        fixedNumOne = FixidityLib.fixed1();
        
        // Inizializzazione variabili
        addressBeneficiary = _addrBeneficiary;
        fixedYears = FixidityLib.newFixed(_years);
        fixedFrequency = FixidityLib.newFixed(_frequency);
        fixedEuroInterest = FixidityLib.newFixed(_interestEuro);
		fixedEuroCapitalValue = FixidityLib.newFixed(_capitalEuro);
        timestampStartDate = DateTimeLib.timestampToParsedTimestamp(_startDate);
		
		// Valori calcolati
        fixedNumCedole = calcNumCedoleFixed();
		fixedEuroCedolaValue = calcEuroCedolaValueFixed();
     ...
     */
    function tryToPay(uint _euroEtherValue) public payable onlyOwner returns(string memory OperationResult) {
        ...
        for (uint i=0; i<paymentsDate.length; i++){
            if((paymentsDate[i].payed == false) && Utility.checkDate(paymentsDate[i].timestampDate, realDaysDeltaPaymentDate)){
                // è possibile procedere con il pagamento
                fixedEuroEtherValue = FixidityLib.newFixed(_euroEtherValue);
                
                if(fixedNumCedole >= fixedNumOne){
        	        // cedole da pagare
                    fixedEtherCedolaValue = calcEtherValueFixed(fixedEuroCedolaValue, fixedEuroEtherValue);
                    realEtherCedolaValue = FixidityLib.transformFixedToRealEtherValue(fixedEtherCedolaValue);
                    if((realEtherCedolaValue > 0) && (address(this).balance >= realEtherCedolaValue)){
                        ...
                        fixedNumCedole = fixedNumCedole - fixedNumOne;
						paymentsDate[i].payed = true;
                        result = "Cedola Payed";
                    } 
                    else {
                        // balance insufficiente, rispondo con la quantità di Ether necessaria da ricaricare
                        result = Utility.strConcat("Insufficient funds - Cedola value: ", Utility.uint2str(realEtherCedolaValue));
                    }
                }
                break;
            }
        ...
        if(fixedNumCedole == 0){
            ...
            fixedEtherCapitalValue = calcEtherValueFixed(fixedEuroCapitalValue, fixedEuroEtherValue);
            realEtherCapitalValue = FixidityLib.transformFixedToRealEtherValue(fixedEtherCapitalValue);
            if((realEtherCapitalValue > 0) && (address(this).balance >= realEtherCapitalValue)){
                ...
                // balance insufficiente, rispondo con la quantità di Ether necessaria da ricaricare
                result = Utility.strConcat("Insufficient funds - Capital value: ", Utility.uint2str(realEtherCapitalValue));
            }
        }
    ...
    function updateSmartBondStaus() public payable onlyOwner {
        if(fixedNumCedole == 0){
            killSmartBond();
        }
    }
    ...
    function getRealEthBalance() public view onlyOwner returns(uint RealEtherBalance) {
    ...
    function sendRealEtherToBeneficiario(uint _realEtherAmount) private {
        addressBeneficiary.transfer(_realEtherAmount);
    }
    
    function calcEtherValueFixed(uint _numerator, uint _denominator) private pure returns(uint) {
        return FixidityLib.newFixed(_numerator/_denominator);
    }
    
    // Calcolo il valore della singola cedola in Euro (in valori fixed point)
    function calcEuroCedolaValueFixed() private view returns(uint) {
        return FixidityLib.newFixed(fixedEuroInterest/fixedNumCedole);
    }
    
    // Calcolo il numero totale di cedole da pagare (in valori fixed point)
    function calcNumCedoleFixed() private view returns(uint) {
        uint numCedolePerYear = calcNumCedoleToPayPerYearFixed();
        return FixidityLib.integer(FixidityLib.multiply(numCedolePerYear, fixedYears));
    }
    
    // Calcolo il numero di cedole per singolo anno Fixed
    function calcNumCedoleToPayPerYearFixed() private view returns(uint) {
        return FixidityLib.integer(FixidityLib.newFixed(FixidityLib.newFixed(12)/fixedFrequency));
    }
     ...
     */
    function calcPaymentsDate(uint _frequency) private {
        uint numCedole = FixidityLib.fromFixed(fixedNumCedole);
        
        for(uint i=1; i<=numCedole; i++){
            paymentsDate.push(PaymentDate(false, DateTimeLib.addMonths(timestampStartDate, _frequency*i)));
        }
    }
    ...
    function zdebugGetFEthBalance() public view onlyOwner returns(uint FixedEtherBalance) {
        return FixidityLib.transformRealEtherValueToFixed(address(this).balance);
    }
    
    // Input Data..
    function zdebugGetFYears() public view onlyOwner returns(uint FixedYears) {
        return fixedYears;
    }
    
    function zdebugGetFFrequency() public view onlyOwner returns(uint FixedFrequency) {
        return fixedFrequency;
    }
    
    function zdebugGetFEuroInterest() public view onlyOwner returns(uint FixedEuroInterest) {
        return fixedEuroInterest;
    }
    ...
    function zdebugGetFEtherCapitalValue() public view onlyOwner returns(uint FixedEtherCapitalValue) {
        return fixedEtherCapitalValue;
    }
    ...
    // Cedole..
    function zdebugGetFNumCedolePerYear() public view onlyOwner returns(uint FixedNumCedolePerYear) {
        return calcNumCedoleToPayPerYearFixed();
    }
    ...
    function zdebugGetFEtherCedolaValue() public view onlyOwner returns(uint FixedEtherCedolaValue) {
        return fixedEtherCedolaValue;
    }
    ...
    function zdebugGetFEuroEtherValue() public view onlyOwner returns(uint FixedEuroEtherValue) {
        return fixedEuroEtherValue;
    }
    
    function zdebugGetPaymentsDate(uint _index) public view onlyOwner returns(string memory PaymentDateObject) { 
        return Utility.strConcat("TimeStamp: ", Utility.uint2str(paymentsDate[_index].timestampDate), " Payed: ", Utility.uint2str(paymentsDate[_index].payed ? 1 : 0));
    }
    
    function zdebugGetBlockTimestamp() public view onlyOwner returns(uint BlockTimeStampNow) {
