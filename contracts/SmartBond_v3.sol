...
library FixidityLib {
    ...
    /**
     *  Il numero intero più piccolo rappresentabile con un valore int256: ((2^255)*(-1))
     */
    ...
    /**
     * Il numero intero più piccolo che può essere convertito in tipo fixed point
    ...
    /**
     * Converte un int256 in un fixed point a 24 decimali
    ...
    /**
     * Converte un fixed point a 24 decimali in un int256 
...
contract SmartBond is Ownable {
    ...
    int256 frequency;
    int256 cedoleToPay; // numero di cedole da pagare
    int256 cedolaValue; 
    int256 interestTotalValue;
    int256 tmp;
    ...
    */
    constructor (address payable _beneficiarioAddress, int256 _frequency, int256 _interestTotalValue) public {
        beneficiarioAddress = _beneficiarioAddress;
        frequency = FixidityLib.newFixed(_frequency);
        creatorAddress = msg.sender;
        interestTotalValue = FixidityLib.newFixed(_interestTotalValue);
        cedolaValue = calcCedolaValue(frequency, interestTotalValue);
        cedoleToPay = FixidityLib.divide((FixidityLib.fixed1()*12), frequency);
    }
    ...
    */
    ...
    function payCedola() public onlyOwner {
        if(cedoleToPay > FixidityLib.fixed1()){
            sendAmountToBeneficiario(uint256(FixidityLib.fromFixed(cedolaValue)));
            FixidityLib.subtract(cedoleToPay, FixidityLib.fixed1());
        }
        if(cedoleToPay == FixidityLib.fixed1()){
    ...
    // Determino il valore della singola cedola (in valori fixed point)
    function calcCedolaValue(int256 _frequency, int256 _interestValue) private returns(int256) {
        cedolaValue = FixidityLib.divide(_interestValue, _frequency);
    }
    
    // Invio ETH al beneficiario
    function sendAmountToBeneficiario(uint256 _payAmount) private {
